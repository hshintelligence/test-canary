// TestCanary — HSH Intelligence agent-native service
// Auto-generated from HSH lighthouse template
import { Hono } from "hono";

const VERSION = "0.1.0";
const SVC_ID = "test-canary";
const SVC_NAME = "TestCanary";
...
app.get("/.well-known/agent.json", (c) => {
  const baseUrl = new URL(c.req.url).origin;
  return c.json({ name: SVC_NAME, description: "Test service for hsh-deploy. Will be deleted in 5 min.", url: baseUrl, provider: { name: "HSH Intelligence", url: "https://github.com/hshintelligence" }, version: VERSION, skills: [], securitySchemes: { x402: { type: "x402", network: NETWORK, asset: "USDC" } } });
});

app.get("/schema.json", (c) => {
  const baseUrl = new URL(c.req.url).origin;
  return c.json({ "@context": "https://schema.org", "@type": "Service", "name": SVC_NAME, "description": "Test service for hsh-deploy. Will be deleted in 5 min.", "url": baseUrl, "provider": { "@type": "Organization", "name": "HSH Intelligence", "url": "https://github.com/hshintelligence" }, "license": "https://spdx.org/licenses/MIT", "serviceType": "test" });
});

app.get("/.well-known/ai-plugin.json", (c) => {
  const baseUrl = new URL(c.req.url).origin;
  return c.json({ schema_version: "v1", name_for_human: SVC_NAME, name_for_model: SVC_ID.replace(/-/g, "_"), description_for_human: "Test service for hsh-deploy. Will be deleted in 5 min.", description_for_model: "Test service for hsh-deploy. Will be deleted in 5 min.", auth: { type: "none" }, api: { type: "openapi", url: `${baseUrl}/openapi.json` }, contact_email: "contact@healingsunhaven.com" });
});
...
app.get("/.well-known/x402.json", (c) => c.json({ x402Version: 2, service: SVC_NAME, description: "Test service for hsh-deploy. Will be deleted in 5 min.", payTo: PAY_TO, network: NETWORK, facilitator: FACILITATOR_URL, routes: [] }));
